...
class AC_Args(PrefixProto, cli=False):
    ...
    env_factor_encoder_branch_input_dims = [27]
    env_factor_encoder_branch_latent_dims = [27]
...
class ActorCritic(nn.Module):
    ...
    def __init__(self, num_obs,
        ...
        for l in range(len(AC_Args.adaptation_module_branch_hidden_dims)):
            if l == len(AC_Args.adaptation_module_branch_hidden_dims) - 1:
                adaptation_module_layers.append(
                    nn.Linear(AC_Args.adaptation_module_branch_hidden_dims[l], AC_Args.env_factor_encoder_branch_latent_dims[0]))
            else:
                adaptation_module_layers.append(
        ...
        actor_layers.append(nn.Linear(total_latent_dim + num_obs_history, AC_Args.actor_hidden_dims[0]))
        ...
        critic_layers.append(nn.Linear(total_latent_dim + num_obs_history, AC_Args.critic_hidden_dims[0]))
    ...
    def update_distribution(self, observations, observation_history, privileged_observations):
    ...
    def act(self, observations, observation_history, privileged_observations, **kwargs):
        self.update_distribution(observations, observation_history, privileged_observations)
        return self.distribution.sample()
    ...
    def act_teacher(self, observations, observation_history, privileged_info, policy_info={}):
    ...
    def evaluate(self, observations, observation_history, privileged_observations, **kwargs):
        latent = self.env_factor_encoder(privileged_observations)
        # value = self.critic_body(torch.cat((observations, latent), dim=-1))
        value = self.critic_body(torch.cat((observation_history, latent), dim=-1))
        return value
