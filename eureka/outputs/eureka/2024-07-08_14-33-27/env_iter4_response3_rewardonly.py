    def compute_reward(self):
        ...
        velocity_reward = 3.0 * (1.0 - vel_error / target_velocity)  # Increased scaling for importance
    
        # Reward for maintaining torso height near 0.34 meters
        target_height = 0.34
        height_error = torch.abs(env.root_states[:, 2] - target_height)
        height_reward = 1.0 * (1.0 - height_error / target_height)  # Normalized scaling
        ...
        orientation_reward = -0.05 * orientation_error  # Reduced harshness
    
        # Penalty for high action rates (difference between consecutive actions)
        action_rate_penalty = 0.00001 * torch.sum(torch.abs(env.actions - env.last_actions), dim=1)  # Reduced penalty
        ...
        dof_pos_penalty = 0.01 * torch.sum((dof_pos_error > 
                                          (env.dof_pos_limits[:, 1] - env.dof_pos_limits[:, 0]) * 0.5).float(), dim=1)  # Slightly reduced
    
        # Penalty for high torques to encourage energy efficiency
        torque_penalty = 0.000001 * torch.sum(env.torques ** 2, dim=1)  # Slightly reduced
