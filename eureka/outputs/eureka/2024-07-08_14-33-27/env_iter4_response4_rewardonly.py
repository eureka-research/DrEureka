    def compute_reward(self):
        ...
        height_reward = 1.0 * torch.exp(-3.0 * height_error ** 2)
        ...
        orientation_reward = 1.0 * torch.exp(-5.0 * orientation_error ** 2)
    
        # Penalty for high action rates (difference between consecutive actions)
        action_rate_penalty = 0.0001 * torch.sum((env.actions - env.last_actions) ** 2, dim=1)
    
        # Penalty for DOF limit violations
        dof_pos_error = torch.abs(env.dof_pos - env.default_dof_pos)
        dof_pos_penalty = 0.02 * torch.sum((dof_pos_error > (0.5 * (env.dof_pos_limits[:, 1] - env.dof_pos_limits[:, 0]))).float(), dim=1)
