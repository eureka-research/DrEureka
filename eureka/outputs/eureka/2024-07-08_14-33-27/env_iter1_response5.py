...
class EurekaReward():
    ...
    def compute_reward(self):
        ...
        height_reward = 2.0 * (1.0 - height_error / target_height)  # Increase importance
    
        # Reward for maintaining orientation perpendicular to gravity
        orientation_reward = -torch.norm(env.projected_gravity[:, :2], dim=1) * 2.0  # Increase importance
    
        # Penalty for high action rates (difference between consecutive actions)
        action_rate_penalty = 0.01 * torch.sum(torch.abs(env.actions - env.last_actions), dim=1)
        ...
        torque_penalty = 0.0001 * torch.sum(env.torques ** 2, dim=1)
        
        # Combine all reward terms
        reward = (
            3.0 * velocity_reward  # Stronger emphasis on achieving target velocity
            + height_reward
            + orientation_reward
            - action_rate_penalty
            - 1.0 * dof_pos_penalty
